        ...
        <div className={theme}>
            <Container fluid className="d-flex flex-column p-0" style={{ height: 'calc(100vh - 60px)' }}>
                <Row className="flex-grow-1 m-0" style={{overflow: 'hidden'}}>
                    <Col md={3} lg={2} className="chat-sidebar border-end d-flex flex-column h-100 p-0">
                        {invites.length > 0 && (
                            <div className="p-3 invite-section border-bottom">
                                <small className="fw-bold text-muted">INVITI</small>
                                <ListGroup variant="flush" className="mt-2 bg-transparent">
                                    {invites.map(inv => (
                                        <ListGroup.Item key={inv.id} className="d-flex justify-content-between align-items-center invite-item rounded mb-1 p-2 border shadow-sm">
                        ...
                        <div className="p-3 d-flex justify-content-between align-items-center">
                            ...
                            <Button size="sm" onClick={() => setShowCreateModal(true)}><i className="bi bi-plus-lg"></i></Button>
                        </div>
                    ...
                    <Col md={9} lg={10} className="chat-content d-flex flex-column p-0 h-100 position-relative">
                        ...
                        {currentTeam ? (
                            <>
                                <div className="p-3 border-bottom d-flex justify-content-between align-items-center shadow-sm" style={{height: '70px'}}>
                                    <h4 className="m-0 fw-bold">
                                        <span className="team-name opacity-50">#</span><span className="team-name">{currentTeam.name}</span>
                                    </h4>
                                    <div className="d-flex gap-2">
                                        <Button className="d-flex align-items-center gap-2 shadow-sm px-3 fw-semibold" onClick={() => setShowInviteModal(true)}>
                                ...
                                <div className="flex-grow-1 p-4 position-relative" style={{overflowY: 'auto', zIndex: 1}}>
                                        ...
                                        return (
                                            <div key={idx} className="d-flex flex-column">
                                                ...
                                                <div className={`mb-2 d-flex flex-column ${isMine ? 'align-items-start' : 'align-items-end'}`}>
                                                    <div className={`p-2 px-3 rounded-3 shadow-sm border ${isMine ? 'bg-success text-white' : 'bg-white'}`} 
                                                        style={{maxWidth: '75%', minWidth: '120px', position: 'relative'}}>
                                                        
                                                        {/* Mostra nome solo se NON è mio */}
                                                        {!isMine && <div className="fw-bold small mb-1">{msg.username}</div>}
